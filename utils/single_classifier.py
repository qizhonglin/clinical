...
from utils.util4sklearn import infer_each_class, draw_feature_importance_LogisticRegression, plot_feature_importance

random_state = 42


def classify(X_train, y_train, X_test, y_test, config, checkpoint_dir,
             X_test_ext=None, y_test_ext=None,
             verbose=False):
    """
    ...
    if not os.path.exists(model_file):
        if config['is_tree']: 
            pipe = Pipeline([('Classifier', tree.DecisionTreeClassifier(random_state=random_state))])
            params = [
                # {
    ...
    if verbose:
        ...
        if classifier.__class__.__name__ in ['LogisticRegression']:
            draw_feature_importance_LogisticRegression(classifier.coef_, config['feature_names'], checkpoint_dir)
        if classifier.__class__.__name__ in ['DecisionTreeClassifier', 
                                             'AdaBoostClassifier', 
                                             'ExtraTreesClassifier', 
                                             'RandomForestClassifier', 
                                             'XGBClassifier']:
            plot_feature_importance(classifier.feature_importances_, config['feature_names'], checkpoint_dir)            

    info_internal = predict(model, X_test, y_test, classes, "internal_test", checkpoint_dir)
    info = {
        'internal test': info_internal
    }

    if X_test_ext is not None and y_test_ext is not None:
        y_test_ext = y_test_ext.astype('int')
        info_external = predict(model, X_test_ext, y_test_ext, classes, "external_test", checkpoint_dir)
        info['external test'] = info_external
...
def predict(model, X_test, y_test, classes, title, save_dir):
    ...
    if len(classes) == 2:
        AUC_internal = SensitivitySpecificityStatistics(y_test, probs[:, 1]).plot_roc(title, save_dir)
